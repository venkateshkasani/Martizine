...
const page = () => {
  ...
  const {data, isLoading} = useQuery({
     queryKey:['eeeData',subject],
     queryFn: () => eeeFiles(subject)
  })
    return (
      ...
      {data?.map((obj:any,index:number) => {
           ...
           return (
            <PDFViewer key={index} src={process.env.NEXT_PUBLIC_BASE_URL + `/public/uploads/${obj.file}`} name={fileName} date={dateString} />
           )
        })}
      </div>
      </div>
